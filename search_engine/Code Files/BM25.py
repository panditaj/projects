...
def calculate_BM_25(k1,B,avg_doclen,r,R,num_docs,k2,qf,d):
...
##                print docid,doclen,tf
            d_temp = {}
            for (docid,doclen,tf) in inverted_list:
                K = k1*((1-B)+(B*doclen/avg_doclen))
                part1 = math.log ((((r+0.5)/(R-r+0.5))/((df-r+0.5)/(num_docs-df-R+r+0.5))),2)
                part2 = ((k1+1)*tf)/(K+tf)
                part3 = ((k2+1)*qf)/(k2+qf)
                bm25_score = part1*part2*part3
                d_temp[docid] = bm25_score
...
def print_dict(get_dict,fout,int_ext_map_dict):
    rank = 1
...
fout = open ("sample_bm25_out.txt", "w")
...
while 1:
    ...
    for key in query_hashmap.keys():
        ...
        while query_word_counter < len_query:
            ...
            #print query[query_word_counter], ctf, df
            get_dict = calculate_BM_25(k1,B,avg_doclen,r,R,num_docs,k2,qf,get_dict)        
            query_word_counter += 1

        print_dict(get_dict,fout,int_ext_map_dict)
