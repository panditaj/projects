...
def calculate_lm_JM(d,word_dict_len,num_docs,num_unique_terms,num_terms,lam):
    ...
    while 1:
        ...
        if not line:
            ...
            for (docid,doclen,tf) in inverted_list:
                Jelinek_Mercer_score = math.log (((1-lam)*(tf/float(doclen)) + lam*(float(ctf)/float(num_terms))),2)
                d_temp[docid] = Jelinek_Mercer_score
...
def update_leftover_values(get_dict,word_dict_len,query,num_unique_terms,lam,num_terms):
    counter = 1
            ...
            if not line:
                ...
                for key,value in get_dict.iteritems():
                    if d_temp.has_key(key):
                        ...
                        get_dict[key] = get_dict[key] + math.log ((lam*(float(ctf)/float(num_terms))),2)
...
fout = open ("sample_lm_jm_out.txt", "w")
...
while 1:
    ...
    for key in query_hashmap.keys():
        ...
        while query_word_counter < len_query:
            ...
            get_dict,word_dict_len = calculate_lm_JM(get_dict,word_dict_len,num_docs,num_unique_terms,num_terms,lam)
            query_word_counter += 1
        get_dict = update_leftover_values(get_dict,word_dict_len,query,num_unique_terms,lam,num_terms)
